<x-app-layout>
    ...
    <div class="py-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 sm:pb-6">
        ...
        @if (session('success'))
            <div class="mb-4 p-3 bg-green text-green-700 rounded">{{ session('success') }}</div>
        @endif
        @if (session('error'))
        ...
        <a href="{{ route('users.create') }}"
            class="inline-block mb-4 px-4 py-2 bg-green-light text-white rounded text-sm">
            + Add User
        </a>
        ...
        <div class="sm:hidden space-y-3">
            @forelse ($users as $user)
                <div class="bg-white shadow rounded-xl p-4">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        @if ($user->id !== auth()->id())
                            <div class="flex items-center gap-3 flex-shrink-0">
                                <a href="{{ route('users.edit', $user) }}" class="flex items-center text-green-dark"
                                ...
                                <form action="{{ route('users.destroy', $user) }}" method="POST"
                                    onsubmit="return confirm('Remove this user?')" class="flex m-0">
                                    @csrf @method('DELETE')
                    ...
                    <div class="mt-2">
                        <span class="px-2 py-0.5 text-[11px] font-medium bg-green-light text-green-dark rounded-full">
                ...
                <tbody>
                    @forelse ($users as $user)
                        <tr class="border-t">
                            ...
                            <td class="p-3">
                                <span class="px-2 py-1 text-xs bg-phekong-light text-phekong-dark rounded">
                                    {{ $user->roles->pluck('name')->map(fn($r) => str_replace('_', ' ', $r))->join(', ') ?: 'No role' }}
                                </span>
                            </td>
                            <td class="p-3 space-x-2">
                                @if ($user->id !== auth()->id())
                                    <a href="{{ route('users.edit', $user) }}"
                                        class="text-green-dark hover:underline">Edit Roles</a>
