            ...
            @forelse ($pending as $req)
                <div class="bg-white shadow rounded-xl p-4">
                    ...
                    <div class="flex items-center gap-2 mt-1">
                        ...
                        <span class="text-sm font-semibold text-phekong-600">{{ $req->requested_quantity }}</span>
                    </div>
                    @if ($req->reason)
                    ...
                    <div class="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                        <form action="{{ route('stock-requests.approve', $req) }}" method="POST" class="flex-1">
                            @csrf
                            <button type="submit" class="w-full flex items-center justify-center gap-1 py-2 bg-phekong-600 text-white rounded-lg text-sm font-medium">
                        ...
                        <tr class="border-t">
                            ...
                            <td class="p-3 space-x-2">
                                <form action="{{ route('stock-requests.approve', $req) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="px-3 py-1 bg-phekong-600 text-white rounded text-xs">Approve</button>
                                </form>
